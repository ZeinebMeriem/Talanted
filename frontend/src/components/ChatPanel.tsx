...
export const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-2 space-y-3 custom-scrollbar" style={{ minHeight: 0 }}>
            ...
            <div key={idx}>
              <div style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                <div style={{
                  ...
                  background: msg.role === 'user' ? '#0d1f30' : '#f8fafc',
                  ...
                  {msg.edits && msg.edits.length > 0 && (
                    <div style={{ fontSize: 11, marginTop: 6, color: msg.role === 'user' ? 'rgba(255,255,255,.6)' : '#94a3b8' }}>
                      {msg.edits.map((e, i) => <span key={i}>📄 {e.file}</span>)}
                    </div>
                  )}
                </div>
              </div>
              <div style={{ fontSize: 10, color: '#94a3b8', marginTop: 3, textAlign: msg.role === 'user' ? 'right' : 'left', paddingLeft: msg.role === 'ai' ? 2 : 0, paddingRight: msg.role === 'user' ? 2 : 0 }}>
                {ts}
              </div>
            </div>
          )
        ...
        {isSending && (
          <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
            <div style={{ padding: '10px 14px', borderRadius: 14, borderBottomLeftRadius: 4, background: '#f8fafc', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ display: 'flex', gap: 3 }}>
                {[0, 1, 2].map(i => (
                  <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#94a3b8', animation: 'bounce 1s infinite', animationDelay: `${i * 0.15}s` }} />
                ))}
              </div>
      ...
      {selectedZone && (
        <div style={{
          ...
          >
            ...
            <span style={{ fontSize: 10, fontWeight: 700, color: '#0369a1', textTransform: 'uppercase', letterSpacing: '0.1em', flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 5, marginBottom: 10, overflowX: 'auto', paddingBottom: 2 }}>
                {TYPOGRAPHY_PRESETS.map(t => (
                  <button
                    ...
                    onMouseLeave={e => { e.currentTarget.style.background = '#fff'; e.currentTarget.style.borderColor = '#e2e8f0' }}
                  >
                    <span style={{ fontSize: 16, fontWeight: t.style.fontWeight as number, fontFamily: t.style.fontFamily, color: '#374151', lineHeight: 1 }}>
                      {t.preview}
                    </span>
                    <span style={{ fontSize: 9, fontWeight: 600, color: '#6b7280', whiteSpace: 'nowrap' }}>{t.name}</span>
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 10 }}>
                {LAYOUT_PRESETS.map((l, i) => (
                  <button
                    ...
                    style={{
                      padding: '5px 10px', borderRadius: 20, border: '1px solid #bbf7d0',
                      background: '#f0fdf4',
      ...
      <div style={{
        ...
        <div style={{
          ...
          onFocusCapture={e => (e.currentTarget as HTMLElement).style.borderColor = '#93c5fd'}
        ...
        >
          <input
            ...
            placeholder={
              projectLock && projectLock.lockedBy
                ? `${projectLock.lockedBy} is editing...`
                : selectedZone ? `Change ${selectedZone.label}…` : 'Command agent (ex: add rounded corner)...'
            }
            value={chatInput}
          ...
          <button
            ...
            style={{
              height: 34, paddingLeft: 14, paddingRight: 14, borderRadius: 8, border: 'none', flexShrink: 0,
              background: (!chatInput.trim() || isSending || !selectedGenerationId || !!projectLock)
                ? '#e2e8f0' : '#0d1f30',
              color: (!chatInput.trim() || isSending || !selectedGenerationId || !!projectLock) ? '#94a3b8' : '#fff',
              cursor: (!chatInput.trim() || isSending || !selectedGenerationId || !!projectLock) ? 'not-allowed' : 'pointer',
              transition: 'all .2s', fontWeight: 700, fontSize: 13,
            }}
          >
            {isSending ? '…' : projectLock ? '🔒' : 'Send'}
          </button>
        </div>
        <p style={{ fontSize: 10, color: '#94a3b8', margin: '6px 0 0 2px' }}>Enter to send · Ctrl+Enter in prompt box to generate</p>
      </div>
    </div>
  )
}
